"""
GUI interface to the make_line_relay_trip_checks program to generate
...
from make_line_relay_trip_checks import make_line_relay_trip_checks
from make_line_relay_trip_checks import std_filenames
...
class TkMakeChecksGUI(ttk.Frame):
    ...
    def doit(self):
        ...
        try:
            ...
            initialfile = 'Trip Checks ' + std + ' ' + std_filenames[std] + \
                                     file_rev + '.docx'
...
def main():
    ...
    if os.path.exists(os.path.dirname(sys.argv[0])):
        logfile = os.path.join(os.path.dirname(sys.argv[0]),
                               'make_trip_checks_gui.log')
    else:
        logfile = 'make_trip_checks_gui.log'
    ...
    try:
        root = tk.Tk()
        TkMakeChecksGUI(root, logger).pack()
        root.mainloop()
